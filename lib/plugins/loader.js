...
class PluginsLoader {
  ...
  async load (isRefresh = false) {
    ...
    // 需要排除的文件夹列表
    const excludedFolders = ['example', 'system', 'other', '.gitignore'];
    // 直接计算个数
    await fs.readdir("./plugins/", (err, files) => {
      if (err) {
        ...
        // 根据文件夹列表排除不需要的文件夹
        const filteredFiles = files.filter(file => !excludedFolders.includes(file));
        logger.info(`加载插件完成[${filteredFiles.length}个]`)
        logger.info('-----------')
      }
    });
  ...
  getPlugins () {
    ...
    for (let val of files) {
      ...
      if (fs.existsSync(`${this.dir}/${val.name}/index.js`)) {
        ...
        // ret.push(tmp)
        // 如果没有app文件就跳过,增加健壮性
        if (!fs.existsSync(`${this.dir}/${val.name}/apps`)) {
          continue
        }
        apps = fs.readdirSync(`${this.dir}/${val.name}/apps`, { withFileTypes: true });
        val.name = val.name + '/apps'
      }
      for (let app of apps) {
