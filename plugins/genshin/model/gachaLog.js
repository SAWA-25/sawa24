...
export default class GachaLog extends base {
  ...
  getPool () {
    ...
    switch (msg) {
      ...
      case '抽奖':
        ...
        break
      case '常驻':
        this.type = this.isSr ? 1 : 200
        this.typeName = '常驻'
        break
      case '武器':
        this.type = this.isSr ? 12 : 302
        this.typeName = this.isSr ? '光锥' : '武器'
        break
      case "光锥":
        this.type = 12
        this.typeName = '光锥'
  ...
  async getUid () {
    ...
    let uidArr = []
    for (let uid of logs) {
      let json = this.isSr ? `${this.path}${uid}/301.json` : `${this.path}${uid}/11.json`
  ...
  randData (data) {
    ...
    if (this.type == 301 || this.type == 11) {
      line = [[
        ...
        { lable: `UP花费${this.isSr ? '星琼' : '原石'}`, num: data.upYs, unit: '' }
      ]]
    }
    // 常驻池
    if (this.type == 200 || this.type == 1) {
      line = [[
        { lable: '未出五星', num: data.noFiveNum, unit: '抽' },
        { lable: '五星', num: data.fiveNum, unit: '个' },
        { lable: '五星平均', num: data.fiveAvg, unit: '抽', color: data.fiveColor },
        { lable: `五星${this.isSr ? '光锥' : '武器'}`, num: data.weaponNum, unit: '个' }
      ], [
        { lable: '未出四星', num: data.noFourNum, unit: '抽' },
    ...
    // 武器池
    if (this.type == 302 || this.type == 12) {
      line = [[
        { lable: '未出五星', num: data.noFiveNum, unit: '抽' },
        { lable: '五星', num: data.fiveNum, unit: '个' },
        { lable: '五星平均', num: data.fiveAvg, unit: '抽', color: data.fiveColor },
        { lable: `四星${this.isSr ? '光锥' : '武器'}`, num: data.weaponFourNum, unit: '个' }
      ], [
        { lable: '未出四星', num: data.noFourNum, unit: '抽' },
        { lable: '四星', num: data.fourNum, unit: '个' },
        { lable: '四星平均', num: data.fourAvg, unit: '抽' },
